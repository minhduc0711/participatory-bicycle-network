...
global {
    ...
    int number_of_test_travels <- 5; // nombre de trajets à prédire donné dnas le modèle 7

	file measures_csv <- csv_file("../results/Marseille/measures_to_predict_" + number_of_test_travels + ".csv",true);
    ...
    init{    	
    	...
    	list<vegetation> list_haie <- vegetation where (each.NATURE = 'Haie');
	    	...
	    	ask environment {
	    				...
	    				to: "../results/Marseille/environment_of_measures_to_predict_" + number_of_test_travels + ".csv" type:"csv" rewrite: false;   
		    }  
...
experiment environment_measures_to_predict type: gui {
}