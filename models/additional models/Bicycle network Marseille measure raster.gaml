...
global {
    ...
    file grid_data <- grid_file('../includes/Marseille/20210527/raster_NO2_202105270.tif');
    ...
    date starting_date <- date("2021-05-27");
	...
	reflex update_pollution_grid when: current_date.minute = 0 and current_date.hour != 0 and hour_count != 23 and save_hourly_pollution_data{
		...
		grid_data <- grid_file('../includes/Marseille/20210527/raster_NO2_20210527' + string(hour_count) + '.tif');
		write("file : " + '../includes/Marseille/20210527/raster_NO2_20210527' + string(hour_count) + '.tif');
    ...
    init{    	
    	...
    	grid_data <- grid_file('../includes/Marseille/20210527/raster_NO2_202105270.tif');
    	write("file : " + '../includes/Marseille/20210527/raster_NO2_20210527' + string(hour_count) + '.tif');
    	matrix my_data <- grid_data as_matrix({pollution_cell max_of each.grid_x + 1, pollution_cell max_of each.grid_y + 1});
