...
global {
	...
	int number_of_sensors <- 1; // nombre de capteurs distribués à la population dans le modèle 5
	
	int number_of_test_travels <- 1; // nombre de trajets à prédire donné dnas le modèle 7
    ...
    init{ 
		...
		//matrice de régression végétation: DATE, bois, foret, haie, NO2
    		...
    		write(ind_buildings_volume);
    		
    		matrix regression_matrix_NO2 <- matrix(ind_buildings_volume, pollution_NO2);
    		matrix regression_matrix_O3 <- matrix(ind_buildings_volume, pollution_O3);
    		matrix regression_matrix_PM10 <- matrix(ind_buildings_volume, pollution_PM10);
    		matrix regression_matrix_PM25 <- matrix(ind_buildings_volume, pollution_PM25);
    		
    		write(regression_matrix_NO2);
    		
    		
    		
    		regression NO2_regression <- build(regression_matrix_NO2);
    		// regression O3_regression <- build(regression_matrix_O3);
    		// regression PM10_regression <- build(regression_matrix_PM10);
    		// regression PM25_regression <- build(regression_matrix_PM25);
    		...
    		loop env over: environment {
    			...
    			associated_measure.predicted_NO2_concentration <- predict(NO2_regression, [env.buildings_volume]);
    			// associated_measure.predicted_O3_concentration <- predict(O3_regression, [env.buildings_volume]);
    			// associated_measure.predicted_PM10_concentration <- predict(PM10_regression, [env.buildings_volume]);
    			// associated_measure.predicted_PM25_concentration <- predict(PM25_regression, [env.buildings_volume]);
    		}
